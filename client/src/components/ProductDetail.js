      ...
      <div className="detail-container">
        ...
        <div className="detail-info">
          ...
          <div className="rating">
            ...
            {item.reviews && `(${item.reviews} reseñas)`}
          </div>
          <div className="price-section">
            <span className="price">{item.precio}</span>
            {item.installments && (
              <span className="installments">
                en {item.installments} cuotas
              </span>
            )}
          </div>
          {item.variants && (
            <div className="variants">
              <p>
                Variante: <strong>{item.variant}</strong>
              </p>
              <div className="variant-options">
                {item.variants.map((v) => (
                  <button
                    key={v}
                    className={
                      v === item.variant ? "variant-btn active" : "variant-btn"
                    }
                  >
                    {v}
                  </button>
          ...
          <div className="seller-shipping">
            {item.shipping && (
              <p>
                <strong>Envío:</strong> {item.shipping}
              </p>
            )}
            {item.stock && (
              <p>
            ...
            {item.seller && (
              <p>
                <strong>Vendido por:</strong> {item.seller}
              </p>
            )}
          </div>
          {item.features && (
